...
describe('email preferences + admin send-now', () => {
  ...
  it('admin can enqueue a weekly/reminder send; non-admins get 403', async () => {
    const weekly = await SELF.fetch(
      'https://server/api/admin/users/bob/send-weekly',
      { method: 'POST', headers: as('admin') },
    );
    expect(weekly.status).toBe(200);
    expect(await weekly.json()).toMatchObject({ queued: true, kind: 'weekly' });
    ...
    expect(await reminder.json()).toMatchObject({ queued: true, kind: 'reminder' });
