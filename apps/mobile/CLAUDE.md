...
| `features/dashboard/` | Join form (commitment choices from `GET /api/join-options`, each annotated with its approximate lot count, collapsing to a single "1 lot" option near the cycle end) or the shown bucket's mishna cards + cycle progress; pull-to-refresh. A **bucket pager** (prev/next `IconButton`s + horizontal swipe, label relative to the current bucket) steps the user's portion relative to their *current* (next-unlearned) bucket via `assignmentProvider(int?)` — `null` is the current `/today`, an `int` is an explicit `?bucket=N`; each caches, and the last value is kept on screen while the next loads (the web client's `keepPreviousData`). prev/next step from the served index the response reports (`bucket`/`bucketCount`/`currentBucket`), bounded by bucket 0 and the last bucket. Mirrors the web client's dashboard pager — keep the two in sync. |
