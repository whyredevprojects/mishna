...
| `features/dashboard/` | Join form (commitment choices from `GET /api/join-options`, each annotated with its approximate lot count, collapsing to a single "1 lot" option near the cycle end) or the selected week's mishna cards + cycle progress; pull-to-refresh. A **week pager** (prev/next `IconButton`s + horizontal swipe, "This week" / "Week of …" label) steps `_selectedWeek` (Sunday-UTC key) over `assignmentByDateProvider(date)` — keyed per week so visited weeks cache; the last value is kept on screen while a new week loads (the web client's `keepPreviousData`). Bounds: prev stops at the cycle's first week, next stops once a week's portion is empty. Mirrors the web client's dashboard pager — keep the two in sync. |
