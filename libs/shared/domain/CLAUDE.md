...
| `MishnaChalakim` | The hand-authored "chelek" (lot) division: 120 sequential, contiguous `MishnaLot`s. `lotsForMesechta(nameEn)` (a mesechta may span several lots) / `getLotByNumber(1..120)` / `allLots()`. |
...
| `AssignmentEngine` | `getAssignment(blocks, date)` — streams the user's mishnayot and slices the week's portion, anchored to `blocks[0].startDate` (offset = `weeksSinceStart × pace`, `pace = ceil(totalSize / weeksRemaining(start))`). `getNextAssignment(blocks, completed, date)` — the same `pace`-sized buckets but selected by **progress, not the calendar**: the first bucket still holding an unlearned mishna (empty once the whole portion is learned). Powers the dashboard's "current mishnayos" and the emails' content. The dashboard's prev/next pager walks buckets **positionally** around that: `getBucketAssignment(blocks, bucketIndex, date)` (the slice at an explicit index; empty past the last), `nextUnlearnedBucket(blocks, completed, date)` (the current index — equals `bucketCount` once finished), and `bucketCount(blocks, date)` (`ceil(totalSize / pace)`). `getWeekAssignment(blocks, weekStart)` — the calendar slice for the week bucket containing `weekStart` (still used by the admin assignments audit view); identical to `getAssignment`'s slice. Stateless. |
...
| `blocksForUser(states, userId)` (`block-projection.ts`) | The blocks `userId` holds, flattened across the given `GroupState[]`. A group's state carries **every** member's blocks, so any code computing one user's portion must filter to that user — this is the single place that does. The web (`apps/server`) and email (`@mishna/email-data`) paths both route through it; the data layer only does I/O (load + `JSON.parse` the group state) and hands it here. Pure. |
...
- **Each group is one full covering of the corpus** (its 120 lots tile it once). `GroupManager` creates additional groups when one fills, so the corpus is covered N times over by N groups.
