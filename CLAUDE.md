...
An app for organizing groups to collectively memorize the entire Mishna by Rosh Chodesh Sivan. Users commit to 1, 2, or 3 mishnayos per week and are auto-assigned a block. Groups are auto-created so the full set of mishnayos is covered by Rosh Chodesh Sivan. The cycle resets each Rosh Chodesh Sivan.
...
- **`AssignmentEngine`** — stateless; `getAssignment(blocks, date)` derives a week's mishnayot on demand (offset = `weeksSinceCycleStart * commitment`).
